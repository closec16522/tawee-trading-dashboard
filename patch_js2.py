...
js_code = r"""      window.renderSignalAlerts = function(history) {
          ...
          topSignals.forEach(sig => {
              ...
              dashHtml += `<div class="signal-row-item" style="padding:10px 14px; background:rgba(15,23,42,0.6); border:1px solid rgba(255,255,255,0.06); border-radius:8px;">
                <span class="signal-pair-name" style="font-weight:800; color:#fff;">${sig.symbol}</span>
                <span class="signal-status-label" style="color:${color};">${dirLabel}</span>
                <span style="font-size:11px; color:#60a5fa; font-weight:700;">Entry ${sig.price} • Conf ${sig.confidence}%</span>
              </div>`;
              
              const notifBadgeReal = isBuy ? "▲ BUY ซื้อ" : "▼ SELL ขาย";
              
              const commonNotifHtml = `<div class="notif-item" onclick="window.selectNotifItem('${sig.symbol}', event)" style="cursor:pointer; padding:8px 10px; background:rgba(255,255,255,0.015); border:1px solid rgba(255,255,255,0.03); border-radius:8px; display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
                <span class="notif-badge ${badgeClass}" style="font-size:10.5px; padding:3px 8px;">${notifBadgeReal}</span>
                <div class="notif-info" style="flex:1; margin-left:12px;">
                  <div class="notif-pair" style="font-weight:700; font-size:13px; color:#fff; margin-bottom:2px;">${sig.symbol}</div>
                  <div class="notif-details" style="font-size:10.5px; color:#94a3b8;">เข้า ${sig.price} · SL ${sig.sl} · TP ${sig.tp}</div>
                </div>
                <span class="notif-prob" style="font-size:13px; font-weight:700; color:#60a5fa;">${sig.confidence}%</span>
              </div>`;
...
print("JS syntax patched PROPERLY")